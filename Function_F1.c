...
int checkWhiteSpace(char number[]);
...
bool characteristic(char numString[], int *c)
{
	...
	index = checkWhiteSpace(numString);
...
bool mantissa(char numString[], int *numerator, int *denominator)
{
	...
	iter = checkWhiteSpace(numString);
...
Algorithm: Loops over white space until first valid character is found
*/
int checkWhiteSpace(char number[])
{
	int iter = 0;
	...
	// returns the position of first non-white space character
	return iter;
}
